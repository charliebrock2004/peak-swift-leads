...
function distribute(names: string[], limit: number): SearchArea[] {
  ...
  if (towns.length === 1) return [{ name: towns[0]!, quota }];
  const wanted = Math.max(4, Math.ceil(limit / 2));
  const extra = Math.min(towns.length, DISCOVERY_SAFETY.maxAreas, wanted);
 ...
 * Small town jobs stay a single request. Regions, nations and larger targets
 * fan out across constituent towns. Every area feeds one candidate pool, so
 * extra areas add coverage rather than competing for slots.
 */
export function planSearch(location: string, limit: number): ResearchPlan {
  ...
  const fanOut = place.kind === "region" || place.kind === "nation" || cap > FAN_OUT_ABOVE;
